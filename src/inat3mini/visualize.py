...
def main():
    ...
    dataset = iNat3_mini(root='../../data/inat3mini', train=False, transform=test_transforms)
    ...
    with torch.no_grad():
        ...
        for col_id in range(fig_cols):
            for j in range(fig_rows):
                ...
                save_input = torch.nn.functional.interpolate(save_input.unsqueeze(0), size=(224, 224), mode='bilinear', align_corners=False).squeeze(0)
                ...
                assign_reshaped = torch.nn.functional.interpolate(assign.data.cpu(), size=(224, 224), mode='bilinear', align_corners=False)
                ...
                for i in range(config['nparts']):
                    img = torch.nn.functional.interpolate(assign_reshaped.data[:, i].cpu().unsqueeze(0), size=(224, 224), mode='bilinear', align_corners=False)
