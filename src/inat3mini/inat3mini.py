...
class iNat3_mini(torch.utils.data.Dataset):
    """
    iNaturalis dataset with 3 species (50 samples each).
    ...
    """
    def __init__(self, root, train=True, transform=None, resize=320):
        """
        Load the dataset.
        ...
        if self._train:
            ...
            assert (len(self._train_data) == 120
                    and len(self._train_labels) == 120)
        else:
            self._test_data, self._test_labels, self._test_parts, self._test_boxes = self._get_file_list(train=False)
            assert (len(self._test_data) == 30
                    and len(self._test_labels) == 30)
    ...
    def __getitem__(self, index):
        ...
        # transform 4 landmarks according to the new shape
    ...
    def _get_file_list(self, train=True):
        ...
        image_path = self._root + '/iNat_3spec_mini/images/'
        id2name  = np.genfromtxt(self._root + '/iNat_3spec_mini/indexes_mini.txt', dtype=str)
        id2lab   = np.genfromtxt(self._root + '/iNat_3spec_mini/indexes_labels_mini.txt', dtype=int)
        id2train = np.genfromtxt(self._root + '/iNat_3spec_mini/train_test_split_mini.txt', dtype=int)
        id2part  = np.genfromtxt(self._root + '/iNat_3spec_mini/parts/part_locs_mini.txt', dtype=float)
        id2box  = np.genfromtxt(self._root + '/iNat_3spec_mini/bboxes_mini.txt', dtype=float)
